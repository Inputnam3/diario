...
exports.HybridNutritionService = void 0;
const common_1 = require("@nestjs/common");
const axios_1 = __importDefault(require("axios"));
class HybridNutritionService {
    constructor(configService) {
        this.logger = new common_1.Logger(HybridNutritionService.name);
        this.spoonacularApiKey = configService.get('SPOONACULAR_API_KEY') || '';
        this.edamamAppId = configService.get('EDAMAM_APP_ID') || '';
        this.edamamAppKey = configService.get('EDAMAM_APP_KEY') || '';
    }
    async extractNutritionFromText(message) {
        try {
    ...
    async extractWithSpoonacular(message) {
        try {
            const response = await axios_1.default.post(`https://api.spoonacular.com/food/detect`, { text: message }, { headers: { 'x-api-key': this.spoonacularApiKey } });
            const detectedFoods = response.data.annotations || [];
            const nutritionData = [];
            for (const food of detectedFoods) {
                // Obtém informações nutricionais detalhadas
                const nutritionResponse = await axios_1.default.get(`https://api.spoonacular.com/food/ingredients/${food.id}/information?amount=${food.amount}&unit=${food.unit}`, { headers: { 'x-api-key': this.spoonacularApiKey } });
                const info = nutritionResponse.data;
                nutritionData.push({
                    food: info.name,
                    quantity: food.amount,
                    unit: food.unit,
                    calories: info.nutrition.nutrients.find((n) => n.name === 'Calories')?.amount || 0,
                    protein: info.nutrition.nutrients.find((n) => n.name === 'Protein')?.amount || 0,
                    carbs: info.nutrition.nutrients.find((n) => n.name === 'Carbohydrates')?.amount || 0,
                    fat: info.nutrition.nutrients.find((n) => n.name === 'Fat')?.amount || 0
                });
            }
    ...
    async extractWithEdamam(message) {
        try {
            const response = await axios_1.default.post(`https://api.edamam.com/api/nutrition-details?app_id=${this.edamamAppId}&app_key=${this.edamamAppKey}`, { ingr: [message] });
    ...
    estimateNutrition(food, quantity, unit) {
        ...
        const baseNutrition = Object.keys(nutritionDatabase).find(key => normalizedFood.includes(key));
        if (baseNutrition) {
        ...
        return {
            calories: quantity * 1.5,
            protein: quantity * 0.05,
            carbs: quantity * 0.1,
            fat: quantity * 0.02
        };
    }
    generateNutritionSummary(nutritionData) {
        ...
        return `🔹 *Resumo Nutricional* 🔹\n\n` +
            ...
            nutritionData
                .map((item) => `• ${item.quantity}${item.unit} de ${item.food}: ${item.calories.toFixed(0)} kcal`)
                .join('\n');
    }
}
exports.HybridNutritionService = HybridNutritionService;
//# sourceMappingURL=hybrid-nutrition.service.js.map